...
\begin{abstract}
We present a compact Stock-Flow Consistent (SFC) accounting model for monetary operations in a modern fiat system with four consolidated sectors: Treasury, central bank, commercial banking system, and households (non-bank private sector). In this model, bank lending creates deposits endogenously (``horizontal money''), fiscal deficits add net financial assets (``vertical money''), and bond operations and QE/QT are portfolio swaps that change the composition of private portfolios without changing private net financial assets. The document is written to match an accompanying interactive web application that posts stylized transactions and updates balance sheets in real time.
\end{abstract}
...
\begin{remark}
This model is intentionally narrow: it abstracts from production, prices, interest flows, bank capital regulation, foreign trade, and non-financial assets. Those are important in full macro models; here we isolate balance sheet mechanics.
\end{remark}
...
\input{figures/balance-sheets.tex}
...
\subsection{Outline of Key Results (To Be Derived)}

In the following subsections, we will:
\begin{enumerate}
    \item Formalize the period-by-period balance sheet updates as matrix equations.
    \item Derive the condition under which a rate hike increases the \emph{real} deficit (accounting for induced inflation).
    \item Show numerical examples with high debt/GDP scenarios matching empirical context from recent policy debates \cite{reuters2022,cnbc2022,bloomberg2024}.
    \item Illustrate multi-period trajectories in the interactive application.
\end{enumerate}
